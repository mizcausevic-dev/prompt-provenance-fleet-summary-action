...
export function toMarkdown(report) {
    ...
    lines.push(`- Approval state: draft=${report.byState.draft} · proposed=${report.byState.proposed} · approved=${report.byState.approved} · deprecated=${report.byState.deprecated} · revoked=${report.byState.revoked}`);
    lines.push(``);
    ...
    for (const r of report.rows) {
        lines.push(`| \`${r.id}\` | ${r.state} | ${r.isRoot ? "✓" : "—"} | ${r.reviewers} | ${r.evaluations} (${r.passingEvaluations}) | ${r.hasApprover ? "✓" : "—"} | ${r.hasContentUri ? "✓" : "—"} |`);
    }
